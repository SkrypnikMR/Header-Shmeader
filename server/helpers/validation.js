...
module.exports = {
    registrationValidation: ({ email, password, firstName, lastName }) => {
        ...
        if (!regExp.firstAndLastNameRegExp.test(firstName)) return { message: 'Invalid firstName', isValid: false };
        if (!regExp.firstAndLastNameRegExp.test(lastName)) return { message: 'Invalid lastName', isValid: false };
