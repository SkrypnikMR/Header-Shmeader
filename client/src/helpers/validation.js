...
export const validation = {
    registrationValidation: ({ email, password, confirm, firstName, lastName }) => {
        ...
        if (!regExp.firstAndLastNameRegExp.test(firstName) || !firstName) return { message: 'Invalid firstName', isValid: false };
        if (!regExp.firstAndLastNameRegExp.test(lastName) || !lastName) return { message: 'Invalid lastName', isValid: false };
